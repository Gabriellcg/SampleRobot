...
#include <typeinfo>
#include <fstream>
#include <iostream> // Incluído para std::cerr
...
NeuralNetwork::NeuralNetwork()
{
    ...
    // A logica de carregar/treinar deve ser chamada aqui
    carregarPesos("pesos_finais.txt"); 
}
...
// ... (restante da função PrintarValores e testarValor) ... (Sem alterações na lógica de Print/Test)
{
    // ... Código da função PrintarValores() ...
    for (p = 0; p < PadroesTreinamento; p++)
    {
...
void NeuralNetwork::salvarPesos(const char* nomeArquivo)
{
    std::ofstream file(nomeArquivo);

    if (file.is_open())
    {
        // Salvar Pesos da Camada Oculta (incluindo o bias - NodosEntrada + 1)
        for (i = 0; i < NodosEntrada + 1; i++)
        {
            for (j = 0; j < NodosOcultos; j++)
            {
                file << PesosCamadaOculta[i][j] << "\n";
            }
        }

        // Salvar Pesos da Camada de Saída (incluindo o bias - NodosOcultos + 1)
        for (i = 0; i < NodosOcultos + 1; i++)
        {
            for (j = 0; j < NodosSaida; j++)
            {
        ...
        printf("\nPesos da Rede Neural salvos com sucesso em: %s\n", nomeArquivo);
        file.close();
    }
    else
    {
        printf("\nERRO: Nao foi possivel salvar os pesos no arquivo %s\n", nomeArquivo);
    }
}

void NeuralNetwork::carregarPesos(const char* nomeArquivo)
{
    std::ifstream file(nomeArquivo);

    if (file.is_open())
    {
        printf("\nCarregando pesos da Rede Neural do arquivo: %s...\n", nomeArquivo);
        ...
        for (i = 0; i < NodosEntrada + 1; i++)
        {
            for (j = 0; j < NodosOcultos; j++)
            {
                if (!(file >> PesosCamadaOculta[i][j]))
                {
                    printf("ERRO ao ler pesos da camada oculta. Arquivo corrompido ou incompleto.\n");
                    file.close();
                    return;
                }
            }
        ...
        for (i = 0; i < NodosOcultos + 1; i++)
        {
            for (j = 0; j < NodosSaida; j++)
            {
                if (!(file >> PesosSaida[i][j]))
                {
                    printf("ERRO ao ler pesos da camada de saida. Arquivo corrompido ou incompleto.\n");
                    file.close();
                    return;
                }
            }
        ...
        printf("Pesos carregados com sucesso! Treinamento ignorado.\n");
        file.close();
    }
    else
    {
        // Se o arquivo não existir, o treinamento precisará ser executado e os pesos salvos.
        printf("\nAVISO: Arquivo de pesos %s nao encontrado. Iniciando treinamento...\n", nomeArquivo);
        treinarValidar(); 
    }
}